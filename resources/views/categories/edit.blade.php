...
<body>
    <div class="container">
        ...
        <form action="{{ route('categories.update', $category->id) }}" method="POST">
            @csrf
            @method('PUT')
            <input type="text" name="name" value="{{ $category->name }}" required placeholder="Nama">
            <input type="text" name="slug" value="{{ $category->slug }}" required placeholder="Slug">
            <textarea name="description" required placeholder="Deskripsi">{{ $category->description }}</textarea>
            <!-- Tombol Submit -->
